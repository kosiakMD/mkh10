...
$link = "http://auto-maxi.com.ua/index.php?keywords=&x=0&y=0&row_by_page=all";
echo $link."<br>";
//$link = "http://auto-maxi.com.ua";
...
//$doc = phpQuery::newDocumentFilePHP($link, $contentType = "text/html;charset=UTF-8");
$doc = phpQuery::newDocumentFileHTML($link/*, $charset = 'utf-8'*/);
...
function normJsonStr($str){
	...
	function getLinks($doc){
		echo "start - "."<br>";
		/*foreach ($doc['a.tovar-name'] as $a){
			$name = pq($a)->text();
			$name = transliterate($name);
			//$name = iconv("cp1251", "utf-8", $name);
			//$img_link = str_replace("w=250&h=250", "w=500&h=500", $img_link);
			echo $name.'<br>';
		}*/
		foreach ($doc['div.tovar-prevu>a>img'] as $a){
			$img_link = pq($a)->attr('src');
			$vowels = array("http://auto-maxi.com.ua/r_imgs.php?thumb=", "&w=250&h=250");
			$name = str_replace($vowels, "", $img_link);
			//echo $name.'<br>';
			$img_link = str_replace("w=250&h=250", "w=500&h=500", $img_link);
			//echo $img_link.'<br>';
			
			//define('DIRECTORY', '/loaded');
